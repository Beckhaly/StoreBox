...
describe('fcfa()', () => {
  it('formate un entier avec séparateurs de milliers', () => {
    expect(fcfa(1_000_000)).toBe('1 000 000 F');
  });
  ...
  it('gère les grands montants (millions)', () => {
    expect(fcfa(10_000_000)).toBe('10 000 000 F');
  });
});
...
describe('fcfaM()', () => {
  it('exprime en millions avec 1 décimale', () => {
    expect(fcfaM(5_000_000)).toBe('5.0 M F');
  });

  it('arrondit correctement', () => {
    expect(fcfaM(1_500_000)).toBe('1.5 M F');
    expect(fcfaM(2_750_000)).toBe('2.8 M F');
  });

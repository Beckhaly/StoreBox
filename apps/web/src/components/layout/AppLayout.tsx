...
function SidebarContent({ user, initiales, onSearchOpen, onNavClick, onLogout, magasins, magasinActif, peutChoisir, onMagasinChange }: SidebarContentProps) {
  return (
    <>
      ...
      {/* Profil + Logout */}
      <div className="px-2 py-2 border-t border-black/[0.07]">
        <div className="flex items-center gap-2.5 px-2.5 py-2 rounded-xl hover:bg-[#F8F7F4] transition-colors group">
          ...
          <div className="min-w-0 flex-1">
            <div className="text-[12px] font-medium truncate text-[#1A1917] leading-tight">{user?.prenom} {user?.nom}</div>
            <div className="font-mono text-[10px] text-[#A8A49E] truncate">{user?.roleLabel}</div>
          </div>
          <button
            onClick={onLogout}
            title="Déconnexion"
            className="opacity-0 group-hover:opacity-100 flex items-center justify-center w-6 h-6 rounded-md hover:bg-red-50 hover:text-red-500 text-[#A8A49E] transition-all flex-shrink-0"
          >
